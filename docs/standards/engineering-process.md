...
This document defines how `kaos-agents` handles features, bugs,
...
- What `kaos-agents` version, platform, Python version, and installation
  method are involved?
- Which extras are installed (`[llm]`, `[mcp]`, `[api]`, `[otel]`,
  `[rerank]`, or any of the per-tool-module extras `[pdf]`, `[office]`,
  `[source]`, `[web]`, `[citations]`, `[tabular]`)?
- Is this Chat / PlanExecute / Research / Findings / Reflexion / Router
  pattern behavior, a memory or recorder issue, an MCP tool issue, the
  FastAPI HTTP surface, or the CLI?
- Is this unit, integration, live, network, slow, benchmark, packaging,
  or docs behavior?
- Is there security sensitivity (cost-cap bypass, permission-policy
  bypass, prompt injection, recorder leakage, multi-tenant isolation,
  credentials)?
...
2. Identify the public API surface affected (CLI, MCP tool surface,
   FastAPI HTTP routes, SSE/JSONL wire schema, `KAOS_AGENT_*` env vars,
   `KaosAgentSettings`).
3. Decide whether a short design note is needed (longer features live
   under `docs/design/`).
4. Implement behind stable boundaries.
5. Add tests at the correct tier. Live-API features need at least one
   `live`-marked integration test.
...
capabilities belong behind extras (`[llm]`, `[mcp]`, `[api]`,
`[otel]`, `[rerank]`, `[pdf]`, `[office]`, `[source]`, `[web]`,
`[citations]`, `[tabular]`); shipped flags must work.
...
5. Check adjacent behavior for the same failure mode (e.g. a
   cost-accounting bug found in one pattern often affects siblings).
...
- Does it affect performance, memory, errors, output ordering, audit
  trail schema, CLI output, MCP tool output, FastAPI route shape, or
  environment-variable behavior?
- Does it need migration guidance?
- Does it require benchmark evidence (BEIR for retrieval changes;
  pairwise Jaccard for findings consistency)?
...
- User-visible impact.
- Public API, CLI, MCP tool surface, FastAPI route, audit-trail schema,
  environment-variable, or package metadata impact.
- Release impact.
- Screenshots or sample output when CLI / tool output / viewer changes.
...
- Do not include secrets, local paths, generated caches, virtual
  environments, build artifacts, or audit-trail JSONL recordings from
  development.
...
```text
feat: add wave-level cost cap to FindingsAgent
fix: preserve session memory across container restart
docs: clarify --max-cost CLI flag
...
- CHANGELOG entry.
- Version bump from `kaos_agents/_version.py`.
