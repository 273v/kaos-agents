...
class BaseAgent(KaosAgent):
    ...
    async def _handle_research_streaming(
        ...
        * ``UsageObserved`` with the filter+synthesis cost rollup
        ...
        2026-05-28 bridge — first attempt at the dynamic-schema
        Extract↔Synthesize path via :meth:`_try_interpret_extraction`.
        When the corpus has ≥ 2 resolvable artifacts AND the agent has
        a ``_runtime`` attached (ChatAgent / ResearchAgent /
        PlanExecuteAgent subclasses), the new path runs first; any
        error or empty memo falls through to the legacy
        findings-dispatch path so this introduces no new failure mode
        — only a new success mode.
        """
        ...
        # ─── 2026-05-28 bridge: try interpret_extraction first ───
        ...
        # ─── legacy findings-dispatch path (fall-through) ───
        findings_result, fallback_text, usage = await self._run_findings_dispatch(
            message, memory, context_items
        )
    ...
    async def _try_interpret_extraction(self, message: str, memory: SessionMemory) -> Any | None:
        """Attempt the dynamic-schema Extract↔Synthesize tool.

        Returns the tool's :class:`ToolResult` on success, or ``None``
        when the bridge declined to run (corpus too small, no
        artifact_ids resolvable, runtime not attached) OR when the tool
        errored at runtime. ``None`` triggers the caller to fall
        through to the legacy findings-dispatch path — bridge errors
        are degraded, not surfaced.

        Gating: requires ≥ 2 resolvable artifact_ids in
        SessionMemory.DOCUMENTS. The interpret_extraction tool is
        designed for per-document typed deliverables; single-doc
        sessions fall through to findings-dispatch which has better
        prose-shape heuristics for one document.

        Conservative bounds for the bridge path: ``max_iters=2``,
        ``budget_usd=0.75``. The standalone tool defaults to 3 / $1.50;
        the bridge halves them so an autonomous misbehavior cannot
        burn the full standalone budget by accident. Callers using the
        tool directly (e.g., via MCP) get the looser limits.
        """
        runtime = getattr(self, "_runtime", None)
        if runtime is None:
            return None
        ...
        for item in docs_items:
            ...
            # Try multiple keys — different upload paths use different
            # conventions. ``hydrated_artifact_id`` is the auto-hydrate
            # marker; ``artifact_id`` is what the SPA upload handler
            # writes; ``uri`` can carry a ``kaos://artifacts/<id>/...``
            # form we strip the prefix from.
            aid = md.get("hydrated_artifact_id") or md.get("artifact_id")
            if not isinstance(aid, str) or not aid.strip():
                uri = md.get("uri") or ""
                if isinstance(uri, str) and uri.startswith("kaos://artifacts/"):
                    rest = uri.removeprefix("kaos://artifacts/")
                    aid = rest.split("/", 1)[0] if rest else ""
        ...
        ctx = KaosContext(session_id=memory.session_id, runtime=runtime)
        ...
        try:
            ...
            logger.warning(
                "interpret_extraction bridge raised, falling back to findings-dispatch: %s",
                exc,
            )
            return None
        ...
        if result.isError:
            text = ""
            if result.content:
                text = str(getattr(result.content[0], "text", "") or "")
            logger.info(
                "interpret_extraction bridge returned error, falling back: %s",
                text[:200],
            )
            return None
        ...
        if not sc.get("memo"):
            logger.info("interpret_extraction bridge produced empty memo, falling back")
            return None
        return result

    async def _emit_interpret_extraction_events(
        self,
        result: Any,
        *,
        research_span_id: str,
        emitter: EventEmitter,
    ) -> AsyncIterator[KaosEvent]:
        """Stream events for a successful interpret_extraction result.

        Emits the same event shape as the legacy findings-dispatch
        path (a synthetic TOOL_CALL span, one CitationFound per cited
        cell, a TextDelta with the memo, UsageObserved, and the
        SUBAGENT span_complete) so downstream consumers — the
        Citations panel, the SSE wire, the corpus-stress judge —
        don't need to special-case the bridge path.
        """
        sc = result.structuredContent or {}
        memo: str = sc.get("memo") or ""
        cost_usd = float(sc.get("cost_usd") or 0.0)
        total_tokens = int(sc.get("total_tokens") or 0)
        extracted = sc.get("extracted") or {}

        tc_span = emitter.span_start(
            SpanSubject.TOOL_CALL,
            name="tool.kaos-agent-interpret-extraction",
            attributes={
                "tool_name": "kaos-agent-interpret-extraction",
                "arguments": (
                    ("iterations_run", int(sc.get("iterations_run") or 0)),
                    ("loop_status", str(sc.get("loop_status") or "?")),
                ),
            },
        )
        yield tc_span

        # One CitationFound per cited cell.
            ...
            for col_id, cell in (row.get("cells") or {}).items():
                ...
                for span in cell.get("spans") or []:
                    if not isinstance(span, dict):
                        continue
                    source_uri = str(span.get("source_uri") or artifact_id or "")
                    quote = str(span.get("quote") or span.get("text") or "")
                    if source_uri or quote:
                        yield emitter.emit(
                            CitationFound,
                            source_uri=source_uri,
                            quote=quote[:500],
                            block_ref=None,
                            finding_id=f"{artifact_id}:{col_id}",
                        )
        ...
        if memo:
            yield emitter.emit(TextDelta, content=memo)
        ...
        yield emit_usage_observed(emitter, usage, source="interpret-extraction-bridge")
        ...
        yield emitter.span_complete(
            ...
            name="research.findings_dispatch",
            attributes={
                "path": "interpret_extraction_bridge",
                "memo_chars": len(memo),
                "rows": int(extracted.get("row_count") or 0),
            },
        )
